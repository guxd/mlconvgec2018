...
CUDA_VISIBLE_DEVICES=$device python3.5 $FAIRSEQ/generate.py --no-progress-bar --path $models --beam $beam --nbest $beam --interactive --workers $threads processed/bin < $output_dir/dev.input.bpe.txt > $output_dir/dev.output.bpe.nbest.txt
